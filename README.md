...
著者検証環境(Gemini無料版)での実証においては、「APIコード生成プロンプト」と「API設計書JSON」及び「Prismaコード」を学習させたLLMに、実際に実装したAPIのコードを与えてテストコードを出力させたところ、各コンポーネントのロジック、バリデーション、および依存関係の呼び出しが適切に行われていることを検証できる品質のテストコードが自動生成でき、プログラマの作業は下記の4点に留まった。
