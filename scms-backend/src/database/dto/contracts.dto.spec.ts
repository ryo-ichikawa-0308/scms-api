import { SelectContractsDto, CreateContractsDto } from 'src/database/dto/contracts.dto';
    ...
    describe('SelectContractsDtoのテスト', () => {
        ...
        describe('異常系', () => {
            test('型違反の入力がある場合', async () => {
                ...
                expect(errors.some(e => e.property === 'quantity' && e.constraints?.isInt)).toBeTruthy();
            });
        });
    });
    describe('CreateContractsDtoのテスト', () => {
        ...
        describe('異常系', () => {
            test('必須項目が未入力の場合 (quantity)', async () => {
                ...
                const errors = await validate(dto);
                expect(errors.length).toBeGreaterThan(0);
                expect(errors.some(e => e.property === 'quantity' && e.constraints?.isDefined)).toBeTruthy();
            });
            test('型違反の入力がある場合 (userServicesId)', async () => {
                ...
                const errors = await validate(dto);
                expect(errors.length).toBeGreaterThan(0);
                expect(errors.some(e => e.property === 'userServicesId' && e.constraints?.isString)).toBeTruthy();
            });
        });
    });
});
