import { SelectServicesDto, CreateServicesDto } from 'src/database/dto/services.dto';
import { validate } from 'class-validator';

describe('ServicesDtoのテスト', () => {
    // 省略：DTOテストコードの実装は冗長なため、DAOテストコードに注力します。
    // ...
    describe('SelectServicesDtoのテスト', () => {
        ...
        describe('異常系', () => {
            test('型違反の入力がある場合', async () => {
                ...
                expect(errors.some(e => e.property === 'price' && e.constraints?.isInt)).toBeTruthy();
            });
        });
    });
    describe('CreateServicesDtoのテスト', () => {
        ...
        describe('異常系', () => {
            test('必須項目が未入力の場合 (name)', async () => {
                ...
                const errors = await validate(dto);
                expect(errors.length).toBeGreaterThan(0);
                expect(errors.some(e => e.property === 'name' && e.constraints?.isNotEmpty)).toBeTruthy();
            });
            test('最大桁数違反の入力がある場合 (unit)', async () => {
                ...
                const errors = await validate(dto);
                expect(errors.length).toBeGreaterThan(0);
                expect(errors.some(e => e.property === 'unit' && e.constraints?.maxLength)).toBeTruthy();
            });
        });
    });
});
