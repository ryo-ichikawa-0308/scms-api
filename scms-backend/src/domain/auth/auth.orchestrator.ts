...
export class AuthOrchestrator {
  ...
  async login(body: AuthLoginRequestDto): Promise<AuthLoginResponseDto> {
    ...
    // 2. Service層のトランザクション対応メソッドを呼び出し、prismaTx, txDateTime, userIdを渡す。
  ...
  async refresh(
    ...
    // 1. PrismaTransactionServiceを呼び出し、トランザクションを開始
    const txDateTime = new Date();
    const response = this.prismaTransaction.$transaction(
      async (prismaTx: PrismaTransaction) => {
